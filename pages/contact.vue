    ...
    <div class="contact-layout">
      ...
      <div class="form-panel">
        ...
        >
          ...
          <input v-if="supportAppSlug" type="hidden" name="app" :value="supportAppSlug" />
          <input v-if="supportType" type="hidden" name="type" :value="supportType" />
...
import { appsIndexEntries } from '~/data/apps'
...
const supportAppName = computed(() => {
  if (!supportAppSlug.value) return ''
  const app = appsIndexEntries.find((entry) => entry.slug === supportAppSlug.value)
  return app?.name ?? supportAppSlug.value
})
...
async function onSubmit() {
  ...
  try {
    const body = encode({
      ...
      ...(supportAppSlug.value ? { app: supportAppSlug.value } : {}),
